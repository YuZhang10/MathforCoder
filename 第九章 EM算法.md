...
\end{align}
$$
对应到GMM算法中，$P(Z|\theta$)表示某一个模型被选中的概率，对应$\pi_1,\pi_2,\pi_3...$；

$P(Y|Z,\theta)$表示已经选中了模型和参数，生成Y的概率，对应$p(Y|\theta)=\prod_{n=1}^N\sum_{i=1}^{K}\pi_i p(x_n|\theta_i)$  
...
- 首先，假设我有数据标签，那我用excel先筛掉小学生的数据，单独用正态分布对大学生建模，用极大似然，就可以很好地估计出μ和σ；再全部筛掉大学生的数据，用线性分布对小学生建模，也能估计出a和b。

- 反过来，假设我有准确的参数。那我可以用似然函数判断出大学生或小学生模型产生该时长的概率。比如某一个用户时长为0h，那他有比较大的概率是小学生，而比较小的概率是大学生，从而得出分类

可我两者都没有，而且我还要同时估计这两个，棘手！
...
EM是目的是最大化似然函数，如果似然函数有上界，那么只要保证每一步似然函数都在增大，算法就一定是收敛的。

P182页定理9.2，收敛到对数似然函数序列的稳定点，并不会收敛到极大值点。所以既不是局部极大值，也不是全局最大值。

解决这一问题的方法——*选取几个不同的初值进行迭代，然后对得到的各个估计值加以比较，从中选择最好的。*
...
- 分组概率
  ...
  $\hat{\gamma_{jk}}$还可以被理解为，第j个点的多少部分被第k个高斯分布支配
  
  这种视角下：整个数据集被K个高斯分布按一定方式均分
